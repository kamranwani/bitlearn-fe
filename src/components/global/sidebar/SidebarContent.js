...
const SidebarContent = () => {
  ...
  return (
    <div className="py-4 text-gray-500 dark:text-gray-400 ">
      ...
      >
        {mode === "dark" ? (
          <img src={logoLight} alt="bitlearn" width="90" className="pl-6" />
        ) : (
          <img src={logoDark} alt="bitlearn" width="90" className="pl-6" />
        )}
      </a>
      <ul className="mt-8">
        {sidebar.map((route) =>
            ...
            <li className="relative" key={route.name}>
              {route?.outside ? (
                <a
                  ...
                  target="_blank"
                  className="px-6 py-4 bg-white inline-flex items-center cursor-pointer w-full text-sm font-semibold transition-colors duration-150 hover:text-green-700 dark:hover:text-gray-200"
                  rel="noreferrer"
                >
